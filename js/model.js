...
function createModelModule() {
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            imageModel.setID(this.imageModels.length);
            var self = this;

            imageModel.addListener(function(imageModel, eventTime) {

                if (imageModel.getState() == 1) {
                    self.removeImageModel(imageModel);
                } else {
                    _.each(
                        self.listeners,
                        function(listener_fn) {
                            listener_fn(IMAGE_META_DATA_CHANGED_EVENT, self, imageModel, new Date())
                        }
                    );
                }
            });

            this.imageModelListeners.push({
                imageModel: imageModel,
                listener_fn: imageModel.listeners[imageModel.listeners.length-1]
            });

            _.each(
                this.listeners,
                function(listener_fn) {
                    listener_fn(IMAGE_ADDED_TO_COLLECTION_EVENT, self, imageModel, new Date());
                }
            );
        },
        ...
        removeImageModel: function(imageModel) {
            ...
            for(var i=0; i<this.imageModels.length; i++) {
                if (imageModel == this.imageModels[i]) {
            ...
            _.each(
                this.imageModelListeners,
                function(imageModelListener) {
                    if (imageModel == imageModelListener.imageModel) {
                        imageModelListener.imageModel.removeListener(imageModelListener.listener_fn);
                    }
                }
            );
            ...
            _.each(
                this.listeners,
                function(listener_fn) {
                    listener_fn(IMAGE_REMOVED_FROM_COLLECTION_EVENT, self, imageModel, new Date())
                }
            );
        },
